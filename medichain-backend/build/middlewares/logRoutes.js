...
export const LogAllIncomingRoutes = (req, res, next) => {
    const start = new Date().getTime();
    console.log(chalk.bold(chalk.bgBlue(` REQUEST  `), chalk.gray(`[${formatTime()}] ::`), chalk.bgYellow(` ${req.protocol.toUpperCase()} ${req.method} `), chalk.blueBright(req.path)));
    res.on("finish", () => {
        const duration = new Date().getTime() - start;
        const endStr = chalk.gray(req.originalUrl) +
            " - " +
